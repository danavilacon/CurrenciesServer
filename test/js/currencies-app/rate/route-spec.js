describe('Rate Routes: ', function() {
    beforeEach(module('currenciesApp.rate'));
	...
	describe('Index route: ', function() {
		beforeEach(inject(
			function($httpBackend) {
				...
				$httpBackend.whenGET('/api/rate').respond([]);
	...
	describe('Create route: ', function() {
		beforeEach(inject(
			function($httpBackend) {
				...
				$httpBackend.whenGET('/api/rate/create').respond([]);
				$httpBackend.whenGET('/api/conversion').respond([]);
			}
		));
	...
	describe('Edit route: ', function() {
		beforeEach(inject(
			function($httpBackend) {
				$httpBackend.whenGET('create-edit.html').respond(200, 'list page');
				$httpBackend.whenGET('/api/rate/1').respond([]);
				$httpBackend.whenGET('/api/conversion').respond([]);
			}
		));
