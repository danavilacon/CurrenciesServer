describe('Conversion Routes: ', function() {
    beforeEach(module('currenciesApp.conversion'));
	...
	describe('Index route: ', function() {
		beforeEach(inject(
			function($httpBackend) {
				...
				$httpBackend.whenGET('/api/conversion').respond([]);
				$httpBackend.whenGET('/api/currencyFrom').respond([]);
				$httpBackend.whenGET('/api/currencyTo').respond([]);
			}
		));
	...
	describe('Create route: ', function() {
		beforeEach(inject(
			function($httpBackend) {
				...
				$httpBackend.whenGET('/api/conversion/create').respond([]);
				$httpBackend.whenGET('/api/currencyFrom').respond([]);
				$httpBackend.whenGET('/api/currencyTo').respond([]);
			}
		));
	...
	describe('Edit route: ', function() {
		beforeEach(inject(
			function($httpBackend) {
				$httpBackend.whenGET('create-edit.html').respond(200, 'list page');
				$httpBackend.whenGET('/api/conversion/1').respond([]);
				$httpBackend.whenGET('/api/currencyFrom').respond([]);
				$httpBackend.whenGET('/api/currencyTo').respond([]);
			}
		));
